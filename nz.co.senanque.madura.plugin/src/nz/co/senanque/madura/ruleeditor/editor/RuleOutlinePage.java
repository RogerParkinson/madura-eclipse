 ...
 */
public class RuleOutlinePage extends ContentOutlinePage implements TOCInterface
{
    private static final Logger log = LoggerFactory.getLogger(RuleOutlinePage.class);
	...
	public RuleOutlinePage(IFile input, TextEditor textEditor)
	{
		super();
		m_input = input;
		m_textEditor = textEditor;
		MaduraPlugin.registerBuilder(m_input.getProject(),RulesBuilder.getId());
	}
	/**  
		...
		SchemaParser schemaParser)
	{
		...
		int lineCount = 0;
		m_parserOffset = 0;
        List<Class<?>> externalFunctions = ProjectPropertyPage.getExternalFunctions(m_input.getProject());
		for (int i = 0; i < r.length; i++)
		{
			...
			try
			{
		        ...
		        ParsePackage parser = new ParsePackage();
		        parser.parse(textProvider);
...
//		if (parent == m_input)
			m_toc.add(element);
			log.debug("Adding TOC name {} start {} end {}",name,adjustedStart,end-start);
		return element;
	}
